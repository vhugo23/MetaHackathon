...
**AC-10** — When an incident is created or updated, a JSON log line is written to stdout containing `incident_id`, `device_id`, `rule_ref`, `severity`, `status`, `outcome` (`CREATED` or `UPDATED`, distinguishing which case occurred), and `timestamp`. *(Implemented as of Day 10 — `timestamp` is exactly the persisted incident's `last_seen_at`, never a fresh clock read; emission is best-effort and strictly post-commit, and never alters any API response schema — proven at the unit, PostgreSQL-application, in-memory-HTTP, and PostgreSQL-HTTP levels.)*
